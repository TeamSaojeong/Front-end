...
const roundUp5 = (m) => (m % 5 === 0 ? m : m + (5 - (m % 5)));
const nowRoundedUp5 = () => {
  const d = new Date();
...
const to12hParts = (d) => {
  let h24 = d.getHours();
  const meridiem = h24 >= 12 ? "오후" : "오전";
  let hour12 = h24 % 12;
  if (hour12 === 0) hour12 = 12;
  return { meridiem, hour12, minute: d.getMinutes() };
};
const to24hStr = ({ meridiem, hour12, minute }) => {
  let h = hour12 % 12;
  if (meridiem === "오후") h += 12;
...
export default function TimeRangePicker({
  ...
  const hourOptions = useMemo(() => {
    ...
    if (cur.meridiem === minParts.meridiem) {
      return HOURS.filter((h) => h >= minParts.hour12);
    }
    // '오전'은 END 편집일 때만 허용되며, 그 중에서도 12시(=00시)만 허용
    if (editing === "end" && cur.meridiem === "오전") {
      return [12];
    }
    // 그 외에는 전체 허용
    return HOURS;
  }, [cur.meridiem, minParts, HOURS, editing]);

  const minuteOptions = useMemo(() => {
    const sameMer = cur.meridiem === minParts.meridiem;
    ...
    if (editing === "end" && cur.meridiem === "오전" && cur.hour12 === 12) {
      return [0];
    }

    // START에서는 오후 11시에서 다음날로 넘어가면 안되므로 sentinel 금지
    if (editing === "start" && cur.meridiem === "오후" && cur.hour12 === 11) {
      return base;
    }

    // 그 외에는 앞으로 한 칸 더 내리면 다음 시간 00으로 롤오버할 수 있도록 sentinel(60) 추가
    return [...base, 60];
  }, [cur.meridiem, cur.hour12, minParts, MINUTES, editing]);

  // 현재 선택이 옵션 밖으로 나가면 즉시 보정
  useEffect(() => {
    if (!merOptions.includes(cur.meridiem)) {
      setCur((p) => ({ ...p, meridiem: merOptions[0] }));
      return;
    }
          ...
          onChange={(v) => {
            if (v === 60) {
              ...
              setCur((p) => {
                ...
                const next = { ...p, meridiem, hour12, minute: 0 };
