  ...
  return (
    <div className="flower-container">
      ...
      <div className="flower-header">
        <div className="flower-title">주차 이용 시간을{"\n"}선택해 주세요</div>

        <div className="flower-meta">
          <div className="flower-row">
            <span className="flower-label">주차 장소 이름</span>
            <span className="flower-value">
              {loading ? "불러오는 중..." : error ? "—" : placeName}
            </span>
          </div>
          <div className="flower-row">
            <span className="flower-label">주차 가능 시간</span>
            <span className="flower-value">
              {loading ? "불러오는 중..." : error ? "—" : openRangesText}
            </span>
          </div>
          <div className="flower-row">
            <span className="flower-label">10분당 주차 비용</span>
            <span className="flower-value">
              {loading
                ? "…"
                : error
        ...
        {/* 아이콘 + 선택 시간 */}
        <div className="flower-chip">
          <img src={clockIcon} alt="" className="flower-chip-icon" />
          <span>{durationText}</span>
        </div>
      </div>

      {/* Wheel Picker (박스 없이 두 줄 가이드만) */}
      <div className="flower-wheel-wrap">
        <div
          className="flower-wheel"
          ref={wheelHRef}
          onScroll={handleScroll("h")}
          aria-label="시간 선택 휠"
        >
          <div className="flower-spacer" />
          {hours.map((hh) => (
            <div className={`flower-item ${h === hh ? "active" : ""}`} key={hh}>
              {hh}
            </div>
          ))}
          <div className="flower-spacer" />
        ...
        <div className="flower-col-suffix">:</div>
        ...
        >
          <div className="flower-spacer" />
          {minutes.map((mm) => (
            <div className={`flower-item ${m === mm ? "active" : ""}`} key={mm}>
              {fmt2(mm)}
            </div>
          ))}
          <div className="flower-spacer" />
        ...
        {/* 가운데 가이드 라인 */}
        <div className="flower-guide-line flower-guide-top" />
        <div className="flower-guide-line flower-guide-bot" />
      </div>

      <div className="flower-bottom">
        <button
          className={`flower-pay ${
